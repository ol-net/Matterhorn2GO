...
{	
	...
	public class DownloadSeriesThumbnails extends EventDispatcher
	{
		...
		public function DownloadSeriesThumbnails()
		{
		...
		{	
			...
			dispatchEvent(new SeriesThumbnailLoadedEvent(SeriesThumbnailLoadedEvent.DOWNLOAD_COMPLETE, url, file, index));
		...
		private function loadDefaultImage(event:IOErrorEvent):void
		{
			...
			{
				file = File.userDirectory.resolvePath(imageName);
				dispatchEvent(new SeriesThumbnailLoadedEvent(SeriesThumbnailLoadedEvent.DOWNLOAD_COMPLETE, thumb, file, index));
			}

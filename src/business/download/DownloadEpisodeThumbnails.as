...
{	
	...
	public class DownloadEpisodeThumbnails extends EventDispatcher
	{
		...
		public function DownloadEpisodeThumbnails()
		{
		...
		{	
			...
			dispatchEvent(new EpisodeThumbnailLoadedEvent(EpisodeThumbnailLoadedEvent.DOWNLOAD_COMPLETE, url, file, index));
		...
		private function loadDefaultImage(event:IOErrorEvent):void
		{
			...
			{
				file = File.userDirectory.resolvePath(imageName);
				dispatchEvent(new EpisodeThumbnailLoadedEvent(EpisodeThumbnailLoadedEvent.DOWNLOAD_COMPLETE, thumb, file, index));
			}
